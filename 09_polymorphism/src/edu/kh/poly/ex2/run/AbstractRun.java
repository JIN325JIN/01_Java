...
public class AbstractRun {
	...
	public static void main(String[] args) {
		...
		Calculator cal = new LYJCalculator();//다형성중 업캐스팅
		...
		//ex)홍길동 계산기 ->이윤진 계산기로 이니셜만 바꿨음.
		
		//인터페이스 특징 
		//1. 인터페이스를 부모 참조변수로 사용하면 다형성중 업캐스팅이 적용되서
		//상속 받은 모든 클래스를 자식 객체로 참조 할 수 있다.
		//-> 이를 이용하여 중요한 메인코드의 수정을 최소화 할 수 있다.
		
		//2.자식 클래스의 공통된 메서드를 강제 구현하라고 하기 때문에
		//모든 자식 클래스가 동일한 형태를 띄게 된다.
		//->이를 이용하여 공동작업( 팀프로젝트)에 필요한 개발환경 조성에 용이하다!
